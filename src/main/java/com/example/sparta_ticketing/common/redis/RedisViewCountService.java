...
public class RedisViewCountService {
    ...
    public boolean hasViewed(Long showId, Long userId) {
        String key = "view:show:" + showId + ":user:" + userId; // key 형식: view:공연ID:유저ID
    ...
    public void increaseViewCount(Long showId, Long userId) {
        if (!hasViewed(showId, userId)) { // 조회 이력이 없다면
            ...
            String viewedKey = "view:show:" + showId + ":user:" + userId; // 중복 방지를 위한 키
    ...
    // 인기 공연 상위 count개를 조회 (조회수 기준 내림차순 정렬)
